...
describe("SEC-031: executiveMetricsRepository.getAll reads the real previous-month snapshot", () => {
  test("an invoice overdue last month but paid off since no longer forges a fabricated growth improvement", async (t) => {
    ...
    const client = await prisma.client.create({ data: { name: `sec031-client-${suffix}`, serviceId } });
    createdClientIds.push(client.id);
    const project = await prisma.project.create({ data: { name: `sec031-project-${suffix}`, clientId: client.id, serviceId } });
    ...
    const snapshot = await prisma.executiveKpiSnapshot.create({
      data: { serviceId, monthStart: prevMonthStart, overdueAmount: 500, pendingAmount: 0 },
    });
