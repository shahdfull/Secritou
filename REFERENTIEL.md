...
points. **SEC-047 ouverte (session du 2026-07-19, `ouvert`)** :
`Task.priority` est modélisé (Prisma/Zod/UI) mais absent de toute
logique métier serveur (jamais tri, notification ni règle) — donnée
purement déclarative ; usage voulu à trancher (décision produit, non
déduite). Voir SEC-044, SEC-045, SEC-047.
