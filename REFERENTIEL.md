...
**SEC-061 ouverte (session du 2026-07-19, `ouvert`, `faible`)** : le
CLIENT ne voit son projet qu'à travers une timeline synthétique en 7
étapes et un brief, jamais le détail des tâches — arbitrage
confidentialité interne déjà cohérent, piste conditionnelle (vue
simplifiée listant les grandes étapes/livrables) si le besoin de
transparence se confirme un jour. Non vérifié par relecture exhaustive
cette session (présenté par le porteur comme un arbitrage déjà connu).
...
d'autres boucles (ex. NPS client). Aucune action requise. Voir
SEC-055, SEC-057, SEC-058, SEC-061, SEC-062.
...
Voir SEC-056. **SEC-059 ouverte (session du 2026-07-19, `ouvert`)** :
un commentaire de tâche n'est jamais modifiable ni supprimable —
`task.routes.ts` ne déclare que `GET`/`POST` sur
`/tasks/:taskId/comments`, aucune route par `commentId`. Décision
produit à trancher avant correctif (politique de réversibilité pour ce
type de contenu). Non corrigée cette session. **SEC-060 ouverte
(session du 2026-07-19, `ouvert`)** : absence de 6 fonctionnalités PM
standard (sous-tâches, dépendances entre tâches, vue calendrier/Gantt,
actions en masse, pièces jointes sur tâche, mentions @ dans les
commentaires) — confirmées absentes par lecture directe du schéma et
du code. Arbitrage de positionnement produit (outil interne vs
concurrent PM généraliste), pas un défaut ; non corrigée cette
session, documentée pour arbitrage futur du porteur. Voir SEC-059,
SEC-060.
