...
`aiConversation.service.ts` → `runConversationTurn` →
`callOllamaWithTools`, SEC-060 : `callOllama` retirée après être devenue
sans appelant réel) et consommé par
...
Outils agrégés du 2026-07-31 (horizon moyen du rapport d'audit, demande
explicite du porteur — multi-sélection, tous retenus) : 5 nouveaux tools
"où j'en sommes", par opposition aux tools "liste-moi X" existants —
tous délégués intégralement aux services déjà scopés qui alimentent les
tableaux de bord REST équivalents, aucune nouvelle requête Prisma
directe dans `aiTools.ts`. `getAgencyOverview` →
`dashboardService.getFullDashboard(serviceId)` (déjà caché/préchauffé,
`dashboard.controller.ts`). `getOverdueProjects`/`getOverdueInvoices` →
filtrent `executiveMetricsService.get(serviceId).risks` sur
`PROJECT_CRITICAL`/`INVOICE_OVERDUE` — ce tableau est déjà plafonné à 20
entrées tous types confondus par `executiveMetricsRepository`, donc ces
deux tools n'ont pas de champ `truncated` (il lirait toujours `false`,
trompeur) ; la limite est documentée dans leur description et dans
`SYSTEM_PROMPT` à la place. `getFreelancerWorkload` →
`timeEntryService.workloadByAssignee(from, to, serviceId)`, fenêtre par
défaut 30 jours, une date malformée du modèle retombe sur ce défaut
plutôt que d'échouer. `getLeadPipeline` → nouvelle méthode
`leadRepository.countByStatus` (`prisma.lead.groupBy`, réutilise
`leadServiceFilter`, extrait de la triple duplication déjà présente dans
`findAll`/`findById`/`findByIdWithProposals` à cette occasion) —
aucune agrégation par statut n'existait nulle part dans le dépôt avant
ce tool ; retourne les 6 statuts explicitement, y compris à zéro.
`server/src/repositories/lead.repository.ts` (module 4.1 CRM, pas
dupliqué dans `perimetre_code` ci-dessous) et
`server/src/services/lead.service.ts` sont touchés par cet ajout mais
restent structurellement rattachés à 4.1 — même logique déjà appliquée
à `cvExtraction.service.ts`, partagé entre modules sans y être listé
deux fois.

Outils d'écriture PROPOSÉE du 2026-07-31 (horizon moyen du rapport
d'audit, demande explicite du porteur). Règle de conception
non-négociable : le modèle n'écrit JAMAIS directement — nouveau module
`server/src/services/aiActionProposals.ts`, 3 tools
(`proposeCreateTask`, `proposeUpdateLeadStatus`, `proposeUpdateTaskStatus`)
qui valident qu'une action est plausible (cible existante, dans le pôle
de l'appelant, transition non déjà interdite) via les mêmes lectures
scopées qu'une vraie écriture utiliserait d'abord
(`projectService.getProjectById`, `leadService.getLead`,
`taskService.getTaskById`), puis renvoient une proposition structurée —
aucune écriture Prisma n'est jamais exécutée par ces 3 tools. La table
de transition des leads (`LEAD_NEXT_STATUSES`, non exportée par
`lead.service.ts`) est dupliquée à titre informatif uniquement, pour
rejeter une proposition manifestement invalide avant qu'elle
n'atteigne l'utilisateur — la vraie barrière reste `lead.service.ts
#updateLead`, revérifiée intégralement à la confirmation ; pour les
tâches, `ALLOWED_TASK_TRANSITIONS` est importé directement du package
partagé `@secritou/shared`, pas dupliqué. La proposition est encodée en
JSON après un marqueur dédié
(`aiConversation.service.ts#encodeActionProposal`,
`\n\n<!--secritou:ai-action-proposal:...-->`) appendu au contenu de
l'`AiMessage` ASSISTANT persisté — survit donc à un rechargement de
page, comme le reste de l'historique de chat. Côté client,
`client/src/features/ai-assistant/actionProposal.ts` parse ce marqueur
(dégrade silencieusement vers « aucune proposition » sur un marqueur
malformé/tronqué, jamais une exception) et
`ActionProposalCard.tsx` affiche une carte de confirmation inline dans
le fil de conversation, dont le bouton « Confirmer » appelle
directement `useCreateTask`/`useUpdateTask`/`useUpdateLeadStatus` — les
mêmes hooks/endpoints REST déjà utilisés partout ailleurs dans
l'application, aucune nouvelle route ni chemin d'écriture parallèle.
Décision produit du porteur : l'état de décision (confirmée/annulée)
n'est pas persisté côté serveur — un rechargement de page réaffiche la
carte dans son état initial actionnable même après une vraie décision
antérieure ; l'entité elle-même (Task créée, Lead/Task mis à jour) est
la trace durable de ce qui s'est réellement passé, pas la carte.

Comparaison demandée par le porteur (2026-07-31) entre les tools de
lecture d'`aiTools.ts` et `search.service.ts`/`search.repository.ts`
(recherche globale, transverse — hors périmètre de ce module) : ce ne
sont pas des doublons à fusionner — mécanismes et objectifs différents
...
CLAUDE.md et la leçon de SEC-094 sur les retries non bornés).
    ...
    perimetre_code:
      ...
      - server/src/services/aiActionProposals.ts
      - server/src/services/aiToolRouter.ts
...
L'export `User` inclut aussi son historique `AiConversation` (titre,
dates, et pour chaque conversation la liste complète des `AiMessage` —
role/content/date), SEC-064 : depuis SEC-059 (tool calling CRM), ce
contenu peut recopier des données CRM (noms de clients, montants,
deadlines) lues au nom de cet utilisateur — un angle mort silencieux
sinon.
...
touchés, conservés pour l'obligation légale de rétention. La branche
anonymisation d'un User supprime aussi (même transaction) ses
`AiConversation` — `AiConversation.userId` est `onDelete: Cascade`,
ce qui supprime en même temps les `AiMessage`/`AiToolCall` enfants
(SEC-064) : anonymiser le seul User laisserait sinon subsister
intactes des données CRM recopiées dans le chat par le tool calling
(SEC-059) — supprimé plutôt qu'anonymisé en place, car un traitement
texte pour retirer les PII d'un `AiMessage.content` en langage libre
n'est jamais garanti fiable, contrairement à l'écrasement d'un champ
structuré. Effacer un
...
| **2026-07-31 (outils agrégés CRM, horizon moyen, même jour)** | **5 nouveaux tools "où j'en sommes" ajoutés sur demande explicite du porteur (multi-sélection, item unique retenu parmi 4 proposés). getAgencyOverview (dashboardService.getFullDashboard), getOverdueProjects/getOverdueInvoices (filtrage de executiveMetricsService.get().risks par type, sans champ truncated — ce tableau est déjà plafonné à 20 entrées en amont, un champ truncated y lirait toujours false), getFreelancerWorkload (timeEntryService.workloadByAssignee, fenêtre 30 jours par défaut), getLeadPipeline (nouvelle méthode leadRepository.countByStatus, prisma.lead.groupBy — aucune agrégation par statut n'existait nulle part avant ce tool). Refactoring connexe sans changement de comportement : la triple duplication du filtre de scope MANAGER (OR serviceId/assignedManagerId) dans findAll/findById/findByIdWithProposals de lead.repository.ts extraite en une fonction leadServiceFilter partagée, réutilisée par countByStatus. SYSTEM_PROMPT mis à jour pour orienter le modèle vers ces outils sur une question d'ensemble plutôt que de composer la réponse à partir de plusieurs listes. Vérifié : server tsc --noEmit/npm run lint (0/0), client tsc --noEmit/npm run lint (12 warnings, exception documentée), suite complète serveur 735/735 (en hausse de 725 — 3 nouveaux tests countByStatus dans lead.repository.test.ts, 7 nouveaux tests dans aiTools.test.ts).** | **AskUserQuestion, choix explicite du porteur : "Outils agrégés CRM (Recommandé)" seul retenu parmi 4 options (SEC-064/RGPD, outils d'écriture proposée, redite avec search.service.ts non retenus pour cette passe). Second AskUserQuestion sur le détail : pipeline de leads construit via une nouvelle méthode repository groupBy plutôt que dérivé de getLeads existant (6 requêtes) ; les 5 tools proposés tous retenus ("tout").** |
| **2026-07-31 (outils d'écriture proposée, même jour)** | **3 nouveaux tools de PROPOSITION d'action ajoutés sur demande explicite du porteur, avec une règle de conception non-négociable actée : le modèle n'écrit jamais directement. Nouveau module server/src/services/aiActionProposals.ts : proposeCreateTask/proposeUpdateLeadStatus/proposeUpdateTaskStatus valident via les mêmes lectures scopées qu'une vraie écriture (getProjectById/getLead/getTaskById), sans jamais toucher Prisma en écriture — testé explicitement (14 tests server, dont plusieurs qui vérifient qu'aucune ligne Task/Lead n'est créée/modifiée par le tool). Proposition encodée en JSON après un marqueur dédié, appendu au contenu de l'AiMessage ASSISTANT persisté (survit à un rechargement de page). Côté client : actionProposal.ts (parsing défensif, dégrade vers "aucune proposition" sur marqueur malformé) et ActionProposalCard.tsx (carte de confirmation inline dans le chat, boutons Confirmer/Annuler) — le clic sur Confirmer appelle directement useCreateTask/useUpdateTask/useUpdateLeadStatus, les mêmes hooks REST déjà utilisés ailleurs dans l'app, aucun nouvel endpoint d'écriture. Décision du porteur : l'état de décision n'est pas persisté serveur (un reload réaffiche la carte actionnable) — l'entité elle-même (Task créée, Lead/Task mis à jour) reste la trace durable. SYSTEM_PROMPT mis à jour pour interdire explicitement au modèle de prétendre avoir exécuté une action lui-même. Vérifié : server tsc --noEmit/npm run lint (0/0), client tsc --noEmit/npm run lint (11 warnings, exception documentée), suite complète serveur 747/747 (en hausse de 735), suite complète client 133/133 (nouveaux fichiers actionProposal.test.ts et ActionProposalCard.test.tsx). Commit `c53a427` : CI GitHub Actions (run 30633960117) rouge sur le seul job `i18n-check` (7 `MISSING_KEY` sous `aiAssistant.proposal.*` — les `t()` de `ActionProposalCard.tsx` utilisaient des défauts inline jamais déclarés dans `client/src/i18n/locales/{fr,en}/translation.json`) ; les 5 autres jobs (`server`, `client`, `contrast-check`, `restore-smoke-test`, `e2e`) verts sur ce même commit. Corrigé par le commit `591cde7` (ajout des 7 clés manquantes dans les deux fichiers de traduction, aucun changement de code) ; run CI 30634755333 confirmé entièrement vert (6/6 jobs, y compris `i18n-check`).** | **AskUserQuestion, choix explicite du porteur : "Outils d'écriture proposée" retenu seul parmi 3 options restantes. Détails tranchés via AskUserQuestion : 3 actions retenues (créer une tâche, changer le statut d'un lead, changer le statut d'une tâche — "autre" non utilisé) ; mécanisme UI "carte inline dans le chat" plutôt que réutiliser ConfirmationDialog.tsx ; proposition persistée dans AiMessage plutôt qu'éphémère en mémoire ; carte figée avec statut visible après décision plutôt que disparition de la carte. Vérification CI demandée explicitement par le porteur (« Vérifier le statut du run CI GitHub Actions 30633960117 ... si rouge, diagnostiquer avant de conclure quoi que ce soit »).** |
| **2026-07-31 (SEC-064, RGPD du tool calling CRM, même jour)** | **SEC-064 résolue : la branche anonymisation d'`eraseUser` (`gdpr.service.ts`) supprime désormais les `AiConversation` de l'utilisateur (`prisma.aiConversation.deleteMany({where:{userId}})`, même transaction que l'anonymisation User/FreelancerProfile) — le cascade `onDelete: Cascade` existant sur `AiConversation.userId` supprime en même temps les `AiMessage`/`AiToolCall` enfants, fermant l'angle mort où un compte anonymisé conservait intact un historique de chat recopiant des données CRM (SEC-059). Suppression choisie plutôt qu'anonymisation en place : retirer des PII d'un texte libre (`AiMessage.content`) par un traitement automatique n'est jamais garanti fiable, contrairement à l'écrasement d'un champ structuré. `exportUser` inclut désormais `aiConversations` (titre, dates, messages complets role/content/date) pour rester fidèle à "l'intégralité des champs personnels détenus" (RG-025). Preuve : `server/test/gdprErasure.test.ts`, 2 nouveaux tests appelant réellement `gdprService.eraseUser`/`exportUser` contre une vraie base (l'un vérifie la suppression de l'`AiConversation` sur anonymisation, l'autre le contenu de l'export). Vérifié : `server tsc --noEmit`/`npm run lint` (0/0). Suite complète serveur 749/749 verte après le changement — un run intermédiaire avait montré 24 échecs non liés à ce changement (collision d'email unique dans `makeManager`/`makeFreelancer`, causée par des lignes `User` orphelines laissées par un kill de process de test précédent dans la même session, pas par `gdpr.service.ts`) ; confirmé non-régression par un second run propre, 749/749.** | **AskUserQuestion, session du 2026-07-31, en réponse directe au choix du porteur de traiter SEC-064 ensuite parmi les chantiers ouverts. Choix explicite « Supprimer les conversations (Recommandé) » (plutôt qu'anonymiser le contenu des messages, ou documenter l'exclusion comme choix assumé) pour la branche anonymisation ; puis « Oui, titres + messages complets (Recommandé) » (plutôt que métadonnées seules, ou laisser l'export hors périmètre) pour `exportUser`.** |
| **2026-07-31 (comparaison search.service.ts vs aiTools.ts, même jour)** | **SEC-065 enregistrée (ouverte) : comparaison ligne à ligne de `search.service.ts`/`search.repository.ts` (recherche globale) et `aiTools.ts` (tools de lecture CRM, SEC-059) demandée par le porteur pour vérifier une redite possible — conclusion : pas de doublon (mécanismes différents, scope réimplémenté localement côté search vs délégué aux services REST côté aiTools), mais une vraie divergence trouvée : `search.repository.ts` bloque tout MANAGER à zéro résultat sur l'annuaire freelancer, alors que `GET /freelancers` (REST direct) et `getFreelancers` (aiTools.ts) autorisent tous deux ce rôle. Comportement search intentionnel et testé (`searchScope.test.ts`, référence historique "SEC-132" dans un commentaire, dont le raisonnement produit d'origine est dans le registre archivé avant le reset du 2026-07-27, non retrouvable). Non corrigé : casser `searchScope.test.ts` sans décision explicite du porteur serait une correction de test non autorisée.** | **AskUserQuestion, session du 2026-07-31, en réponse directe au choix du porteur de comparer ces deux fichiers ensuite parmi les chantiers ouverts. Choix explicite « Documenter comme constat, ne pas corriger (Recommandé) » plutôt qu'aligner search sur le comportement REST/aiTools, ou clore sans anomalie.** |
