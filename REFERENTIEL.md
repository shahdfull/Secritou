...
**SEC-055/SEC-057 ouvertes (session du 2026-07-19, `ouvert`)** :
rapport développeur full-stack + designer UI/UX sur ce module. F4 (hook
`useCreateProject` mort) déjà résolu par SEC-046 avant ce rapport,
aucune action. SEC-055 : aucun bouton « Ajouter une tâche »
contextualisé sur l'onglet Tâches dès que le projet contient déjà une
tâche (le bouton template n'apparaît qu'à l'état vide) ;
`ProjectMeetingsTab.tsx` n'a ni édition ni suppression de réunion et
sa liste n'est pas paginée (confirmé : ni le client ni le serveur
n'ont de route `PUT`/`DELETE` par réunion). SEC-057 : boutons icône
sans `aria-label` systématique, absence d'aide contextuelle expliquant
la disparition du bouton « Nouveau projet » (SEC-039/046), cadence de
réunion visible uniquement dans l'onglet Réunions, absente de l'en-tête
de la fiche. Non corrigés cette session (décision du porteur : « F1
seul, urgent »). SEC-058 (`info`) : le dialogue d'archivage (SEC-046)
explicite honnêtement son irréversibilité — bonne pratique à
généraliser, aucune action requise. Voir SEC-055, SEC-057, SEC-058.
...
**SEC-053/SEC-054/SEC-056 ouvertes (session du 2026-07-19,
`ouvert`)** : SEC-053, le sélecteur Projet du formulaire de création
(`pageSize: 100`) devient incomplet et silencieux au-delà de 100
projets actifs. SEC-054, le sélecteur Assigné à liste tous les rôles
(y compris CLIENT) sans filtrage, alors que le serveur rejette une
assignation CLIENT en 422 `INVALID_ASSIGNEE_ROLE` — découvert
seulement après soumission. SEC-056, `TasksListView.tsx` n'offre
qu'un filtre statut + recherche (manquent projet/assigné/échéance) et
utilise une grille CSS à colonnes fixes sans variante mobile,
contrairement à la grille responsive de `ProjectsPage.tsx`. Non
corrigés cette session (décision du porteur : « F1 seul, urgent »).
Voir SEC-053, SEC-054, SEC-056.
