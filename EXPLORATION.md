...
| 4.6 Portail client | Serveur (intégral) : `clientPortal.service.ts`, `clientPortal.repository.ts`, `clientPortal.controller.ts`, `serviceRequest.service.ts`, `serviceRequest.repository.ts`, `approval.service.ts`, `approval.repository.ts`, `approval.controller.ts`, `customQuestion.service.ts`. Client (intégral, 2 passes — 2026-07-16) : `ApprovalsClientPage.tsx`, `ServiceRequestsClientPage.tsx`, `QuestionsClientPage.tsx`, `ClientDashboardPage.tsx`, `ProposalsClientPage.tsx`, `ProjectsClientPage.tsx`, `OnboardingClientPage.tsx`, `SeoReportPage.tsx`, `components/ProjectTimeline.tsx`, `ServiceRequestsAdminPage.tsx` (932 l.), `ApprovalsPage.tsx` (448 l.), `AdminQuestionsPage.tsx`, `DocumentsClientPage.tsx`, `ClientBriefPage.tsx`, `InvoicesClientPage.tsx`, `ClientProfilePage.tsx` (lu sous 4.14, module partagé) | `approval.routes.ts`, `approval.validator.ts` (existence confirmée, contenu non relu ligne à ligne cette session — voir §4.6, ajoutés au `perimetre_code:` sur la base de leur usage réel), `client-portal/OnboardingClientPage.tsx`'s dépendance `useClientOnboardingByClientId` (hook, non ouvert) | lu |
